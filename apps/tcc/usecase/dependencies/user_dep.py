...
from apps.tcc.usecase.usecases.users.user_create_uc import CreateAdminUserUseCase, CreateUserUseCase, CreateStaffUserUseCase  # ADDED CreateStaffUserUseCase
...
class UserDependencyContainer:
    ...
    async def get_create_staff_user_uc(self) -> CreateStaffUserUseCase:  # ADDED
        key = 'create_staff_user'
        if key not in self._use_cases:
            user_repo = await self.get_user_repository()
            self._use_cases[key] = CreateStaffUserUseCase(user_repository=user_repo)
        return self._use_cases[key]
    ...
    async def get_all_user_use_cases(self) -> Dict[str, Any]:
        return {
            ...
            'create_staff_user': await self.get_create_staff_user_uc(),  # ADDED
